...
public class ReqresAPITest extends TestBase {
    ...
    protected DslHttpSampler getHttpSamplerForInitAudit(String id) {
        String url = getInitAuditUrl()
                + id;
        return jmeterUtil.getBaseHttpSampler("post Init audit", url, HTTPConstants.POST);
    }

    @Test
    void initAudit() throws IOException {
        TestPlanStats stats = JmeterDsl.testPlan(
                threadGroup(10,10, counter("ID"),
                        getHttpSamplerForInitAudit("${ID}"),
    ...
    @Test
    void initAudit2() throws IOException {
        TestPlanStats stats = JmeterDsl.testPlan(
                threadGroup().rampTo(10, Duration.ofSeconds(30)).holdIterating(10)
                        .children(counter("ID"))
                        .children(getHttpSamplerForInitAudit("${ID}"))
                        .children(resultsTreeVisualizer())
        ).run();
        assertEquals(0, stats.overall().errorsCount());
...
//
//        assertEquals(user.contact.email, response.data.email);
//    }
//
//    @Test
